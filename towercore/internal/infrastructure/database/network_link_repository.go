...
func (r *networkLinkRepository) Create(ctx context.Context, link *domain.NetworkLink) error {
	if link.LinkID == "" { link.LinkID = uuid.NewString() }
	now := time.Now().UTC()
	if link.CreatedAt.IsZero() { link.CreatedAt = now }
	if link.UpdatedAt.IsZero() { link.UpdatedAt = now }
...
func (r *networkLinkRepository) Update(ctx context.Context, link *domain.NetworkLink) error {
	...
	n, _ := res.RowsAffected(); if n == 0 { return sql.ErrNoRows }; return nil
}
...
func (r *networkLinkRepository) GetByID(ctx context.Context, id string) (*domain.NetworkLink, error) {
	...
	if err == sql.ErrNoRows { return nil, nil }; if err != nil { return nil, err }; return link, nil
}
...
func (r *networkLinkRepository) List(ctx context.Context, limit, offset int) ([]*domain.NetworkLink, int, error) {
	...
	for rows.Next() { link, e := scanNetworkLink(rows); if e != nil { return nil, 0, e }; links = append(links, link) }
	if err := rows.Err(); err != nil { return nil, 0, err }
	var total int
	if err := r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM network_links`).Scan(&total); err != nil { return nil, 0, err }
...
func (r *networkLinkRepository) GetLastSnapshot(ctx context.Context, linkID string) (*domain.LinkMetricSnapshot, error) {
	var s domain.LinkMetricSnapshot; var inOct,outOct,inErr,outErr,inDis,outDis int64
	...
	if err != nil { if err == sql.ErrNoRows { return nil, nil }; return nil, fmt.Errorf("get last link snapshot: %w", err) }
	s.LinkID=linkID; s.InOctets=uint64(maxInt64(inOct)); s.OutOctets=uint64(maxInt64(outOct)); s.InErrors=uint64(maxInt64(inErr)); s.OutErrors=uint64(maxInt64(outErr)); s.InDiscards=uint64(maxInt64(inDis)); s.OutDiscards=uint64(maxInt64(outDis)); return &s,nil
}

func maxInt64(v int64) int64 { if v < 0 { return 0 }; return v }
