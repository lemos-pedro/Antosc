...
func (h *SiteEnvironmentHandler) collect(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.Unmarshal(payload, &batch); err == nil {
		if err := h.service.CollectMultipleEnvironment(r.Context(), batch); err != nil {
	...
	if err := json.Unmarshal(payload, &env); err != nil {
		apierror.BadRequest(w, "expected a site environment object or array")
		return
	}
...
func (h *SiteEnvironmentHandler) list(w http.ResponseWriter, r *http.Request) {
	siteID, ok := queryUUID(w, r, "site_id")
	if !ok {
		return
	}
...
func (h *SiteEnvironmentHandler) GetSiteEnvironmentStatus(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		apierror.BadRequest(w, "invalid site id")
		return
	}
...
func setEnvironmentBooleanFilters(r *http.Request, f *domain.SiteEnvironmentFilter) {
	if r.URL.Query().Get("high_temp") == "true" {
		n := 35.0
		f.InternalTempCMin = &n
	}
	if r.URL.Query().Get("high_humidity") == "true" {
		n := 80.0
		f.HumidityPctMin = &n
	}
	if r.URL.Query().Get("door_open") == "true" {
		b := true
		f.DoorOpen = &b
	}
	if r.URL.Query().Get("mains_down") == "true" {
		b := false
		f.MainsPowerOk = &b
	}
	if r.URL.Query().Get("ups_on_battery") == "true" {
		b := true
		f.UpsOnBattery = &b
	}
	if r.URL.Query().Get("smoke_detected") == "true" {
		b := true
		f.SmokeDetected = &b
	}
}
...
func filterEnvironment(items []*domain.SiteEnvironment, f *domain.SiteEnvironmentFilter) []*domain.SiteEnvironment {
	...
	for _, item := range items {
		if f.InternalTempCMin != nil && (item.InternalTempC == nil || *item.InternalTempC < *f.InternalTempCMin) {
			continue
		}
		if f.HumidityPctMin != nil && (item.HumidityPct == nil || *item.HumidityPct < *f.HumidityPctMin) {
			continue
		}
		if f.DoorOpen != nil && (item.DoorOpen == nil || *item.DoorOpen != *f.DoorOpen) {
			continue
		}
		if f.MainsPowerOk != nil && (item.MainsPowerOk == nil || *item.MainsPowerOk != *f.MainsPowerOk) {
			continue
		}
		if f.UpsOnBattery != nil && (item.UpsOnBattery == nil || *item.UpsOnBattery != *f.UpsOnBattery) {
			continue
		}
		if f.SmokeDetected != nil && (item.SmokeDetected == nil || *item.SmokeDetected != *f.SmokeDetected) {
			continue
		}
